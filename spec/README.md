...
**Status:** rev 2.21 **Owner:** kenesparta **Target:** `kenesparta.dev` and `auruming.com` — four Rust services on one host (C3, met exactly)
...
| 5  | [Resource Specification](05-resources.md)  | Providers, backend, `projects.yml`, instance, firewall, DNS, DNSSEC delegation (§5.6.1), the `auruming.com` estate (§5.12–5.13), HTTP/3 (§5.14), origin-failure pages (§5.15) |
...
| 12 | [Gotchas](12-gotchas.md)                   | G1–G30 — failure modes not obvious from any single file          |
