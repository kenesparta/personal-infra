...
- **The accept-encoding flags on `disabled_plus_geo` are not cache tuning** (G30). They are the only thing that
  forwards the viewer's `Accept-Encoding` to an origin; with them off — as `CachingDisabled` ships — Caddy's Go
